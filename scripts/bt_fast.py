"""Fast single-symbol backtest — 2500 candles. Run: python bt_fast.py ETH/USDT"""
import asyncio
import sys
import os
...
async def run(symbol: str, timeframe: str = "4h", limit: int = 2500):
    ...
    for i in range(warmup, len(df)):
        ...
        _sym_ov = config.trading.symbol_overrides.get(symbol, {})
        if _sym_ov:
            if _sym_ov.get("adx_min") is not None and (ind.adx or 0) < _sym_ov["adx_min"]:
                continue
            # Block specific setup types (e.g. ["SELL_continuation"])
            _blocked = _sym_ov.get("block_setup_types", [])
            if _blocked:
                _st = f"{setup.direction.upper()}_{setup.setup_type}"
                if _st in _blocked:
                    continue
            tp_tmp = trade_engine.build_trade_plan(
            ...
            if tp_tmp.is_valid and tp_tmp.sl != 0:
                _sl_pct = abs(float(ind.close) - tp_tmp.sl) / float(ind.close) * 100
                if _sym_ov.get("max_sl_pct") is not None and _sl_pct > _sym_ov["max_sl_pct"]:
                    continue
            if _sym_ov.get("max_atr_pct") is not None:
                _atr_pct = (ind.atr or 0) / float(ind.close) * 100
                if _atr_pct > _sym_ov["max_atr_pct"]:
                    continue
        ...
        _min_q = config.trading.symbol_overrides.get(symbol, {}).get("min_quality")
        if _min_q is not None and features.overall_setup_quality < _min_q:
            continue
